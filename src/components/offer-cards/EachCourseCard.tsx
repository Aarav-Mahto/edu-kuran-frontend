...
export default function EachCourseCard() {
  ...
  const cardWidth = width * 0.75; // smaller for better layout
  const cardHeight = width * 0.55; // keep aspect ratio nice
  ...
  return (
    <View className="rounded-xl border border-[#088000] py-3">
      ...
      <View className="mb-2 flex-row items-center justify-between px-4">
        <Text className="text-xl font-bold text-[#088000]">Tajweed Course</Text>
        <TouchableOpacity
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{ width: cardWidth, height: cardHeight, marginRight: 12 }}>
            <TutsCard {...item} />
          </View>
        )}
        horizontal
